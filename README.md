...
Please note: the website’s backend is hosted on Render’s free tier. Navigating and performing CRUD functionalities on the application is therefore very, very slow, **especially** on your first visit as the site requires “warming up” before it becomes _marginally_ faster. You could wait up to 2 minutes before being navigated to a page after performing a CRUD function, so it’s important not to resend a fetch request by re-clicking a button/link.
